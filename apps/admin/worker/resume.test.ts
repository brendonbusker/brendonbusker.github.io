...
const path = "apps/site/src/data/resume.json";
import content from "../../site/src/data/resume.json";

describe("résumé publication API", () => {
  ...
  async function request(payload?: unknown, expectedSha = "current-sha") {
    ...
    const response = await worker.request(
      payload ? "/api/publish" : "/api/published/resume",
      {
  ...
  it("loads the latest published résumé through its allowlisted singleton path", async () => {
  ...
  it("publishes validated fields using the loaded SHA, independently of posts", async () => {
    const edited = { ...content, headline: "Updated headline" };
    const { response, writes } = await request(edited);
    expect(response.status).toBe(200);
    expect(writes).toHaveLength(1);
    expect(writes[0]!.payload.sha).toBe("current-sha");
    expect(writes[0]!.payload.message).toBe("cms: update web resume");
    expect(
      JSON.parse(Buffer.from(writes[0]!.payload.content, "base64").toString()),
    ).toEqual(edited);
  });
  it("rejects missing versions, stale versions and invalid content before writing", async () => {
